...
import { ToolDefinitionDto } from '../../core/models/tools-workspace.model';
 ...
 */
@Component({
  selector: 'app-tool-type-select',
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [Select, FormsModule],
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => ToolTypeSelectComponent),
      multi: true
    }
  ],
  template: `
    <p-select
      ...
      appendTo="body"
      [fluid]="true"
      emptyFilterMessage="לא נמצאו סוגי כלים"
      emptyMessage="אין סוגי כלים"
    />
  `,
  styles: `
    :host {
    ...
    :host ::ng-deep .tool-type-select.p-select,
...
export class ToolTypeSelectComponent implements ControlValueAccessor {
  /** Already-loaded tool types — filtered purely client-side. */
  readonly options = input<ToolDefinitionDto[]>([]);
  readonly placeholder = input('הקלידו לחיפוש סוג כלי...');
